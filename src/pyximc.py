...
# Вариант-костыль Сержа
def load_libximc():
    """
    Загружает libximc.dll в зависимости от разрядности системы.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    if sys.maxsize > 2**32:
        dll_path = os.path.join(base_dir, "win64", "libximc.dll")
    else:
        dll_path = os.path.join(base_dir, "win32", "libximc.dll")
...
#lib = ximc_shared_lib()
#dll_path = os.path.abspath("libximc.dll")  # Получаем полный путь к DLL
#lib = WinDLL(dll_path)
lib = load_libximc()
